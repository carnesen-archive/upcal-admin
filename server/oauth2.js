...
module.exports = function(config){
  ...
  // START middleware
  // Middleware that makes the user's credentials available in the request as
  // 'req.oauth2client'. If no credentials are available, then 'req.oauth2client'
  // will be undefined.
  ...
  // Begins the authorization flow. The user will be redirected to Google where they
  // can authorize the application to have access to their basic profile information.
  // Upon approval the user is redirected to '/oauth2callback'.  If the 'return' query
  // parameter is specified when sending a user to this URL they they will be redirected to
  ...
  // Completes the authorization flow.  When the user approves application access at
  // Google's authorization page, Google returns the user to this URL.  This handler will
  // obtain the user's credentials (access and refresh tokens), save the credentials and
  // user's profile information to the session and then redirect the user to the 'return'
